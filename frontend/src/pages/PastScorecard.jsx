...
const PastScorecard = () => {
  ...
  const downloadPdf = async () => {
    try {
      const res = await api.get(`/matches/${matchKey}/report/pdf`, { responseType: 'blob' });
      const url = window.URL.createObjectURL(new Blob([res.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `match-${matchKey}.pdf`);
  ...
  return (
    <div className="page-container">
      <div className="scorecard-header glass-card">
        ...
        <div className="sc-result">{scorecard.result}</div>
